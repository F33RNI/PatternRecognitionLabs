...
8. В файле `main.py` в `DATASET_PATH` укажите путь к папке PetImages внутри распакованного датасета
9. Начните запись видео
10. Запустите обучение, используя только процессор:
   ```shell
   CUDA_VISIBLE_DEVICES="-1" python main.py
   ```
11. Продемонстрируйте на видео процесс обучения (в консоли и в TensorBoard)
12. Продемонстрируйте, что загруженность видеокарты 0 (через `nvidia-smi` или через диспетчер задач)
13. Дождитесь окончания обучения, переодически показывая процесс на видео
14. Запустите обучение, используя CUDA:
    ```shell
    python main.py
    ```
   ...
   XLA_FLAGS=--xla_gpu_cuda_data_dir="/opt/cuda/" python main.py
   ```
15. Продемонстрируйте на видео процесс обучения (в консоли и в TensorBoard)
16. Продемонстрируйте, что загруженность видеокарты высокая (через `nvidia-smi` или через диспетчер задач)
17. Покажите что процесс обучения идёт быстрее (надеюс)
18. Дождитесь окончания обучения
19. Сравните графики обучения наа процессоре с графиками обучения на видеокарте (из TensorBoard)

### Работа 5. Облачные APIокажите на видео ответ от Gemini (и переведите по необходимости)
